...
public class RecipeMachineReader extends YamlReader<AdvancedCustomMachine> {
    ...
    public AdvancedCustomMachine readEach(String s) {
        ...
        if (capacity < 0) {
            Debug.error(file, section, "MissingConfiguration error 'Source' (capacity)", 0, Integer.MAX_VALUE);
            return null;
        }

        int energy = section.getInt("energyPerCraft", 0);
        if (energy < 0) {
            Debug.error(file, section, "MissingConfiguration error '' (energyPerCraft)", 0, Integer.MAX_VALUE);
            return null;
        }
